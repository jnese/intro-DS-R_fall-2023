...
#### 2.1. Download the *studentInfo.csv*, *studentAssessment.csv*, and *assessments.csv* data from the [course website](https://jnese.github.io/intro-DS-R_fall-2023/data_list.html).
...
#### 3.1. Join the *studentassessment* data and the *assessments* data, keeping all rows in *studentassessment*. Specify to join by `id_assessment` and `date`. *Note* that the `date` column has a different name in *studentassessment* (`date_submitted`). Name the joined data *assess_full*. Filter *assess_full* to remove `NA` values in the `code_module` column. 
...
#### 3.2. Join the *studentinfo* data with the newly created *assess_full* data, keeping all rows in the *assess_full* data. Name the joined data *student*.
